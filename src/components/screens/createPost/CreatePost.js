 ...
 const  CreatePost = ()=> {
   ...
   // posting data to database
    function post(){
        ...
        .then(res => {
            console.log(res);
            M.toast({ html : res.data.message, classes : "#81c784 green lighten-2 rounded" });
        })
        .catch(err => { 
    ...
    const postImage = ()=> {
        ...
        .then(data => {
            console.log(data);
            if(data.error){
                M.toast({ html : data.error.message + "  and fields" ,classes: '#c62828 red darken-3 rounded' });
                return;
            } else{
        ...
        .catch(err => {
            M.toast({ html : err + ", Internet DisConnected" ,classes: '#e57373 red lighten-2 rounded' });
        })
    }
    ...
    return (
       <div className="card postcard input-filed">
           ...
           <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn waves-effect mt-3 waves-light #64b5f6 blue lighten-1" >Create post </button>
            </form>
       </div>
    )
}
